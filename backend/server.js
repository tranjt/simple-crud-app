...
mongodb.MongoClient.connect(dbUrl, function(err, db) {
    ...
    app.get("/api/games/:_id", (req, res) => {
        // mongodb _id is of object type need to make new ObjectID with _id string from req
    ...
    app.post("/api/games", (req, res) => {
        ...
        if (isValid) {
            ...
            db.collection("games").insert({ title, cover}, (err, result) => {
                if (err) {
                    res.status(500).json({ errors: { global: "Something when wrong "}});
                } else {
    ...
    app.use((req, res) => {
        res.status(404).json({
            errors: {
                global: "Still working on it. Please try again later when we have implented it"
            }
        });
    });
